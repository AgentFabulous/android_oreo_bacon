...
void BTA_AvOffloadStartRsp(tBTA_AV_HNDL hndl, tBTA_AV_STATUS status)
{
    ...
    if ((p_buf = (tBTA_AV_API_STATUS_RSP *) osi_malloc(sizeof(tBTA_AV_API_STATUS_RSP))) != NULL)
    {
                    ...
                    UINT8 *p_codec_info, UINT8 num_protect, UINT8 *p_protect_info)
{
    ...
    if ((p_buf = (tBTA_AV_API_RCFG *) osi_malloc(sizeof(tBTA_AV_API_RCFG) + num_protect)) != NULL)
    {
        p_buf->hdr.layer_specific   = hndl;
...
void BTA_AvProtectReq(tBTA_AV_HNDL hndl, UINT8 *p_data, UINT16 len)
{
    ...
    if ((p_buf = (tBTA_AV_API_PROTECT_REQ *) osi_malloc(sizeof(tBTA_AV_API_PROTECT_REQ) + len)) != NULL)
    {
        p_buf->hdr.layer_specific   = hndl;
...
void BTA_AvProtectRsp(tBTA_AV_HNDL hndl, UINT8 error_code, UINT8 *p_data, UINT16 len)
{
    ...
    if ((p_buf = (tBTA_AV_API_PROTECT_RSP *) osi_malloc(sizeof(tBTA_AV_API_PROTECT_RSP) + len)) != NULL)
    {
        p_buf->hdr.layer_specific   = hndl;
...
void BTA_AvVendorCmd(UINT8 rc_handle, UINT8 label, tBTA_AV_CODE cmd_code, UINT8 *p_data, UINT16 len)
{
    ...
    if ((p_buf = (tBTA_AV_API_VENDOR *) osi_malloc(sizeof(tBTA_AV_API_VENDOR) + len)) != NULL)
    {
...
void BTA_AvMetaCmd(UINT8 rc_handle, UINT8 label, tBTA_AV_CMD cmd_code, BT_HDR *p_pkt)
{
    ...
    if ((p_buf = (tBTA_AV_API_META_RSP *) osi_malloc(sizeof(tBTA_AV_API_META_RSP))) != NULL)
    {
        p_buf->hdr.event = BTA_AV_API_META_RSP_EVT;
        p_buf->hdr.layer_specific   = rc_handle;
        p_buf->p_pkt = p_pkt;
