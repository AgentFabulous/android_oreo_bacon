...
TEST(RingbufferTest, test_new_simple) {
  ...
  ASSERT_TRUE(rb != NULL);
  EXPECT_EQ(4096U, ringbuffer_available(rb));
  EXPECT_EQ(0U, ringbuffer_size(rb));
  ringbuffer_free(rb);
}
...
TEST(RingbufferTest, test_insert_basic) {
  ...
  EXPECT_EQ(10U, ringbuffer_size(rb));
  EXPECT_EQ(6U, ringbuffer_available(rb));

  uint8_t peek[10] = {0};
  size_t peeked = ringbuffer_peek(rb, peek, 10);
  EXPECT_EQ(10U, ringbuffer_size(rb)); // Ensure size doesn't change
  EXPECT_EQ(6U, ringbuffer_available(rb));
  EXPECT_EQ(10U, peeked);
...
TEST(RingbufferTest, test_insert_full) {
  ...
  EXPECT_EQ(5U, added);
  EXPECT_EQ(0U, ringbuffer_available(rb));
  EXPECT_EQ(5U, ringbuffer_size(rb));
...
TEST(RingbufferTest, test_multi_insert_delete) {
  ringbuffer_t *rb = ringbuffer_init(16);
  EXPECT_EQ(16U, ringbuffer_available(rb));
  EXPECT_EQ(0U, ringbuffer_size(rb));
  ...
  EXPECT_EQ(2U, ringbuffer_available(rb));
  EXPECT_EQ(14U, ringbuffer_size(rb));
  ...
  EXPECT_EQ(7U, popped);
  EXPECT_EQ(9U, ringbuffer_available(rb));
  ASSERT_TRUE(0 == memcmp(content2, peek, peeked));
  ...
  EXPECT_EQ(8U, added);
  EXPECT_EQ(1U, ringbuffer_available(rb));
  ...
  EXPECT_EQ(13U, ringbuffer_available(rb));
